...
# It is designed to run on a single GPU node with a smaller depth-8 model.

# 1) Example launch (simplest):
# bash runs/speedrun-single-gpu.sh
# 2) Example launch in a screen session (because the run takes ~3 hours):
# screen -L -Logfile runs/speedrun-single-gpu.log -S speedrun-single-gpu bash runs/speedrun-single-gpu.sh
# 3) Example launch with wandb logging, but see below for setting up wandb first:
# WANDB_RUN=speedrun-single-gpu screen -L -Logfile runs/speedrun-single-gpu.log -S speedrun-single-gpu bash runs/speedrun-single-gpu.sh
...
# d8 model for a single-GPU speed run
torchrun --standalone --nproc_per_node=1 -m scripts.base_train -- --depth=8 --target-param-data-ratio=9.5 --device-batch-size=16 --fp8 --run=$WANDB_RUN

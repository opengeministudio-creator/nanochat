...
# It is designed to run the speedrun flow on a single GPU.

# 1) Example launch (simplest):
# bash runs/speedrun-single-gpu.sh
# 2) Example launch in a screen session:
# screen -L -Logfile runs/speedrun-single-gpu.log -S speedrun-1gpu bash runs/speedrun-single-gpu.sh
# 3) Example launch with wandb logging, but see below for setting up wandb first:
# WANDB_RUN=speedrun-1gpu screen -L -Logfile runs/speedrun-single-gpu.log -S speedrun-1gpu bash runs/speedrun-single-gpu.sh
...
#    `WANDB_RUN=d26 bash speedrun-single-gpu.sh`
if [ -z "$WANDB_RUN" ]; then
...
torchrun --standalone --nproc_per_node=1 -m scripts.base_train -- --depth=24 --target-param-data-ratio=9.5 --device-batch-size=16 --fp8 --run=$WANDB_RUN
